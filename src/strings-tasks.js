...
function removeLastOccurrences(str, value) {
  const charindex = str.lastIndexOf(value);
  if (charindex < 0) return str;
  const ptone = str.substring(0, charindex);
  const pttwo = str.substring(charindex + value.length);
  return ptone + pttwo;
}
...
function formatTime(minutes, seconds) {
  ...
  if (minutes < 10) min = min.padStart(2, '0');
  if (seconds < 10) sec = sec.padStart(2, '0');
  return `${min}:${sec}`;
}
...
function invertCase(str) {
  ...
  arr.forEach((element, index) => {
    if (arr[index].charCodeAt(0) > 64 && arr[index].charCodeAt(0) < 91) {
      arr[index] = element.toLowerCase();
    } else if (
      arr[index].charCodeAt(0) > 96 &&
      arr[index].charCodeAt(0) < 123
    ) {
...
function extractNameFromTemplate(value) {
  const arr = value.replace(/[^a-z0-9/\s/]/gi, '').split(' ');
...
function encodeToRot13(str) {
  ...
  arr.forEach((el, i) => {
    if (
      (arr[i].charCodeAt(0) > 64 && arr[i].charCodeAt(0) < 78) ||
      (arr[i].charCodeAt(0) > 96 && arr[i].charCodeAt(0) < 110)
    ) {
      arr[i] = String.fromCharCode(el.charCodeAt(0) + 13);
    } else if (
...
function getCardId(value) {
  ...
  let mark = value.substring(1);
  if (value.length > 2) {
    val = value.substring(0, 2);
    mark = value.substring(2);
  }
  let res = 0;
  const multiplicator = marks.indexOf(mark);
  if (Number.isNaN(Number(val))) {
    res = LetterValues[val] + 13 * multiplicator - 1;
    return res;
  }
  res = Number(val) + 13 * multiplicator - 1;
  return res;
}
